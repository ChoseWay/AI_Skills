...
2. **核心目标** —— 编号列表，每条一个功能域；写到「契约级」细节（对外接口行为、权限规则、数据去向、关键交互定稿），让后来者不用翻代码就知道边界。
...
7. **当前阶段** —— 按日期倒序或正序的**阶段记录** + 末尾一条**迭代规范**（见第三节）。
8. **文档索引** —— 其余文档登记表。

## 三、阶段记录（当前阶段）的写法

每完成一轮有持久价值的工作追加一条 `- YYYY-MM-DD：…`，一条写清：**做了什么（定稿 / 实现 / 修复）→ 关键实现位置（文件 / 表 / 接口）→ 验证方式与结果（tsc / 冒烟 N/N / 浏览器实测要点）→ 发布状态（本机 / Docker / NAS）→ 踩坑与规避**。示例：
...
迭代规范（放在当前阶段末尾，长期有效）：**本机开发（类型检查零错误）→ 本机 Docker 验证 → 确认通过后最后一步才发布 NAS**。
