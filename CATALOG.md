...
| project-guide | `claude/project-guide` | ProjectGuide 工作法（开工先对照 / 缺失先建 / 变更即同步 / 文档登记 / 首行标注）+ 全局工作规则细则（编码 / 输出 / 设计 / 飞书 / 技能仓库 / NAS 部署 / 安全）+ ProjectGuide 模板 + 合规自检 | 新建或补建 ProjectGuide、新项目初始化文档体系、检查合规、把工作规范教给另一个 agent / 新机器、「按我的全局规则 / 千往工作流」 | 自制（2026-08-23，源自 ~/.claude/CLAUDE.md 与 ChoseWayManager 实践） | Claude（agent 无关，可 `link-skills.ps1 -CrossLink @{'project-guide'='codex'}` 给 Codex） | 原文快照在 `global/`，用 sync-global-rules.ps1 同步 |
